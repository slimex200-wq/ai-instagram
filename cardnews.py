...
def main():
    ...
    # 1. 뉴스 수집 (소셜 주력 + RSS 폴백)
    print("[1/5] AI 뉴스 수집 중...")
    articles = []

    try:
        from social_collector import collect_social
        social = collect_social(max_count=30)
        if social:
            articles = social
            print(f"  → 소셜 {len(social)}개 수집 (주력)")
    ...
    # 카드 데이터 저장 (post-text job에서 참조)
    cards_export = {
        "cards": [
            {k: v for k, v in card.items() if k not in ("thumbnail_b64", "banner_b64")}
            for card in content["cards"]
        ],
        "cover_headline": content.get("cover_headline", ""),
        "trend_summary": content.get("trend_summary", ""),
    }
    (output_dir / "cards_data.json").write_text(
        json.dumps(cards_export, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(f"  → 카드 데이터: cards_data.json")
